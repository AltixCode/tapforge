...
describe("Paywall", () => {
  ...
  it("starts the carousel on the first benefit and disables the previous arrow", async () => {
    const { getByText, getByLabelText } = await renderWithProviders(
      <Paywall />,
    );
    expect(
      getByText(t("benefitCounter", { current: 1, total: 4 })),
    ).toBeTruthy();
    expect(
      getByLabelText(t("previousBenefit")).props.accessibilityState?.disabled,
    ).toBe(true);
  });

  it("advances the counter when the next arrow is pressed", async () => {
    const { getByText, getByLabelText } = await renderWithProviders(
      <Paywall />,
    );
    await fireEvent.press(getByLabelText(t("nextBenefit")));
    expect(
      getByText(t("benefitCounter", { current: 2, total: 4 })),
    ).toBeTruthy();
  });
