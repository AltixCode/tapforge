...
describe("Settings", () => {
  ...
  it("reports a successful restore", async () => {
    ...
    await waitFor(() =>
      expect(alert).toHaveBeenCalledWith(t("restoredTitle"), t("restoredBody")),
    );
  });

  it("shows a busy indicator on the restore row while restore is in flight", async () => {
    jest.spyOn(Alert, "alert").mockImplementation(() => {});
    let resolveRestore!: (value: "purchased" | "none" | "error") => void;
    const restore = jest.fn(
      () =>
        new Promise<"purchased" | "none" | "error">((resolve) => {
          resolveRestore = resolve;
        }),
    );
    usePremiumStore.setState({ restore });
    const { getByLabelText } = await renderWithProviders(<Settings />);
    const row = getByLabelText(t("restorePurchases"));

    expect(row.props.accessibilityState.busy).toBeFalsy();
    await fireEvent.press(row);
    expect(
      getByLabelText(t("restorePurchases")).props.accessibilityState.busy,
    ).toBe(true);
  ...
  it("reopens the consent form where Google requires that entry point", async () => {
    useAdsConsentStore.setState({
      consent: { canServeAds: true, offerPrivacyOptions: true },
    });
    const { getByText } = await renderWithProviders(<Settings />);
  ...
  it("switches the theme preference", async () => {
    ...
    await waitFor(() =>
      expect(
        getByLabelText(t("themeDark")).props.accessibilityState.selected,
      ).toBe(true),
    );
  });
